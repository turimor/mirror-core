...
use Mirror\Http\Request;
use Mirror\Http\Response;
...
class Mirror {
	...
	private function header_callback($ch, $headers){
		...
		if(preg_match('/HTTP\/1.\d+ (\d+)/', $headers, $matches) && stripos($headers, '200 Connection established') === false){
			...
			$event = new MirrorEvent(array('request' => $this->request, 'response' => $this->response, 'proxy' => &$this));
	...
	private function write_callback($ch, $str){
		...
		$this->dispatcher->dispatch('curl.callback.write', new MirrorEvent(array(
	...
	public function forward(Request $request, $url){
		...
		$this->dispatcher->dispatch('request.before_send', new MirrorEvent(array('request' => $this->request, 'response' => $this->response)));
		...
		$this->dispatcher->dispatch('request.complete', new MirrorEvent(array('request' => $this->request, 'response' => $this->response)));
